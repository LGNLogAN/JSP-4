			...
			<%
				try{
					...
					String sql = " select al.pcode , ap.name , to_char(sum(ap.cost*al.amount) , '999,999') as ang , " +
								 " to_char(sum(ap.cost*al.amount) , '999,999') as total1" +
					...
					%>
						<tr>
							<td colspan=2>상품별 판매액 총합</td>
					<%
					
					String sql1 = "select to_char(sum(ap.cost * al.amount), '999,999,999') as total1 " +
                            "from ai_product_3 ap, ai_salelist_3 al, ai_shop_3 ash " +
                            "where al.pcode = ap.pcode and ash.scode = al.scode";
					ResultSet totalRs = stmt.executeQuery(sql1);
					
					if (totalRs.next()) {
					    out.print("<td>" + totalRs.getString("total1") + "원</td></tr>");
					}
